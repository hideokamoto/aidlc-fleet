...
|----|-------------|-------------------|---------------|---------------------|
| NFR3.1 | `Channel.plugins[]` and `Lockfile.plugins[]` comparison (used by `VersionGate`, `DriftDetector`, `PluginManager`) must run in linear time relative to plugin count — no quadratic blowup as the plugin set grows. | "Tens of plugins per channel" — this stage's own illustrative estimate `[assumption]`; no v0.1 section or approved artifact states a plugin-count figure | Linear, driven by team adoption of more plugins over time | Algorithmic requirement only — no infrastructure scaling applies to a local CLI process |
